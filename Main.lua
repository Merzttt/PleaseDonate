...
queueonteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/Merzttt/PleaseDonate/main/Main.lua'))()")
local Flux = loadstring(game:HttpGet"https://raw.githubusercontent.com/Merzttt/PleaseDonate/main/Main.lua")()
...
local sNames = {"textUpdateToggle", "textUpdateDelay", "serverHopToggle", "serverHopDelay", "colorBox", "goalBox", "webhookToggle", "webhookBox", "danceChoice", "thanksMessage", "signToggle", "customBoothText", "signUpdateToggle", "signText", "signColorBox", "autoThanks", "autoBeg", "begMessage", "begDelay", "fpsLimit", "render", "thanksDelay", "vcServer", "randomToggle", "rainbowLetters", "webhookshop","webhookErr"}
local sValues = {true, 30, true, 0, "0.196078, 0.803922, 0.196078", 5, false, "", "Disabled", {"Thank you", "Thanks!", "ty :)", "tysm!"}, false, "GOAL: $C / $G", false, "your text here", "1, 1, 1", true, false, {"Please donate", "I'm so close to my goal!", "donate to me", "please"}, 300, 60, false, 3, false, true, false, false, false}
...
local function update(ok)
    ...
    if getgenv().settings.textUpdateToggle and getgenv().settings.customBoothText and ok ~= "sign" then
        ...
        boothText = tostring('<font color="#'.. colll ..'">'.. text.. '</font>')
...
-- local customBoothText = boothTab:Textbox("Booth Text", "Customize what your booth says\n$C = Current, $G = Goal, \\n = Newline, 221 Character Limit", false, getgenv().settings.customBoothText, function(t)
--     if #t > 221 then
--         Flux:Notification("Text too long!", tostring(#t.. "/221 Characters used"), "Okay")
--     else 
...
local customBoothButton = boothTab:Button("Edit Booth Text", "Customize what your booth says\n$C = Current, $G = Goal, 221 Character Limit", function()
...
if signPass then
    ...
    local signText = signTab:Button("Sign Text", "Customize what your sign says\n$C = Current, $G = Goal, \\n = Newline, 221 Character Limit", function()
        if not game:GetService("CoreGui").FluxLib:FindFirstChild("EditBooth") then
            local clonedGui = game:GetService("Players").LocalPlayer.PlayerGui.ScreenGui.EditBooth:Clone()
            clonedGui.Parent = game:GetService("CoreGui").FluxLib
            clonedGui.Buttons.Settings:Destroy()
...
local thanksBox = chatTab:Textbox("Add thank you message", "Add thank you messages\nPress enter to save", true, "", function(t)
    if settingsLock then return end
    thanksDropdown:Add(t)
    table.insert(getgenv().settings.thanksMessage, t)
    saveSettings()
end)

local tymsg = {"CLEAR ALL"}
for i, v in pairs(getgenv().settings.thanksMessage) do table.insert(tymsg, v) end
thanksDropdown = chatTab:Dropdown("Remove thank you message", tymsg, function(t)
    if t == "CLEAR ALL" then
        begDropdown:Clear()
...
begDropdown = chatTab:Dropdown("Remove begging message", bmsg, function(t)
    if t == "CLEAR ALL" then
        begDropdown:Clear()
        begDropdown:Add("CLEAR ALL")
...
local webhookshop = webhookTab:Toggle("Server Hop", "Server Hop Notifications", false, function(t)
    if settingsLock then return end
    getgenv().settings.webhookshop = t
    saveSettings()
end)

local webhookErr = webhookTab:Toggle("Errors", "Error Notifications", false, function(t)
    if settingsLock then return end
...
local danceDropdown = otherTab:Dropdown("Dance", {"Disabled","1","2", "3"}, function(t)
    if settingsLock then return end
